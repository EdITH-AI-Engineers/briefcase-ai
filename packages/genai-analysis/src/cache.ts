...
async function createCache(
  ...
  const expiresAt = new Date(Date.now() + ANALYSIS.cacheTtlSeconds * 1000).toISOString();
  console.log(
    `  ${label} cache created: ${name} (${tokens} tokens, ttl ${ANALYSIS.cacheTtlSeconds}s, expires ${expiresAt})`,
  );
